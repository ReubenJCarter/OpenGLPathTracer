...
"vec3 RayIntersectBVHStacklessCount(vec3 rayOrig, vec3 rayDir, out int triIndex, out vec4 hit)"
"{"
"	vec4 hitTemp;"
"	vec3 rayDirInv = vec3(1.0f, 1.0f, 1.0f) / rayDir;"
"	float dist = 1e10;"
...
"		nodeIndex = skipInx;"
"	}"
"	vec3 col = mix(vec3(0.0f, 0.0f, 1.0f), vec3(0.0f, 1.0f, 0.0f), stepCount);"
"	col = col.z < 0.0001f ? mix(vec3(0.0f, 1.0f, 0.0f), vec3(1.0f, 0.0f, 0.0f), stepCount - 1.0f) : col;"
"	return col.y < 0.0001f ? mix(vec3(1.0f, 0.0f, 0.0f), vec3(1.0f, 1.0f, 1.0f), stepCount - 2.0f) : col;"
...
"	int triangleIndex;"
"	vec4 hit;"
"	finalColor += RayIntersectBVHStacklessCount(rayOrig, rayDir, triangleIndex, hit);"
/*
